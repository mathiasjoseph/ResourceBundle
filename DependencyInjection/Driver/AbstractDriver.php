...
use Miky\Component\Resource\Factory\TranslatableFactoryInterface;
...
abstract class AbstractDriver implements DriverInterface
{
    ...
    public function load(ContainerBuilder $container, MetadataInterface $metadata)
    {
        ...
        if ($metadata->hasContexts()){
            foreach ($metadata->getContexts() as $name => $context){
                if (isset($context['controller'])){
                    $this->addContextController($container, $metadata, $context, $name);
                }
            }}
    ...
    protected function setClassesParameters(ContainerBuilder $container, MetadataInterface $metadata)
    {
        ...
        if ($metadata->hasContexts()){
            foreach ($metadata->getContexts() as $name => $context){
                if (isset($context['controller'])){
                    $container->setParameter(sprintf('%s.controller.%s.%s.class', $metadata->getApplicationName(), $name, $metadata->getName()), $context['controller']);
    ...
    protected function addContextController(ContainerBuilder $container, MetadataInterface $metadata, $context, $contextName)
    {
        $definition = new Definition($context['controller']);
        $definition
            ->setArguments([
                $this->getMetadataDefinition($metadata),
                new Reference('miky.resource_controller.request_configuration_factory'),
        ...
        $container->setDefinition($metadata->getServiceContextId('controller', $contextName), $definition);
    }
    /**
     * @param ContainerBuilder $container
     * @param MetadataInterface $metadata
